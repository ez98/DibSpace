...
from StudySpace.serializers import BuildingSerializer, StudentSerializer, RoomSerializer, ReservationSerializer, StudySpaceSerializer
...
def buildingApi(request, id=0): 
    if request.method == 'GET': #GET retrieves data from the server 
        ...
        buildings = Building.objects.get(building_tag = building_data['building_tag']) #choose the building to update using the tag 
